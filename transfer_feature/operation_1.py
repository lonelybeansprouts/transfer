...
class Convolution2D_pad(Layer):
    ...
    def call(self, inputs):   
        ...
        if (self.source_features):
            ...
            self.kernel = Concatenate(axis=2)([self.kernel_addition,self.kernel])
...
class Convolution2D_test(Layer):
    """Abstract nD convolution layer (private, used as implementation base).
    ...
    def __init__(self, 
        ...
        super(Convolution2D_test, self).__init__(**kwargs)
        self.rank = rank
        self.filters = filters
        self.kernel_size = conv_utils.normalize_tuple(kernel_size, rank, 'kernel_size')
        self.strides = conv_utils.normalize_tuple(strides, rank, 'strides')
    ...
    def call(self, inputs): 
        # inputs=inputs[0]
        # source_features=inputs[1]
        # #特征缩放  
        # bz,h,w,c = self.input_shape_temp
        # self.out_features = []
        ...
        # self.out_features.append(inputs)
        # print('feature:',K.int_shape(inputs))
        # inputs = Concatenate(axis=-1)(self.out_features)
        # print('concatenate feature:',K.int_shape(inputs))
        print('kernel:',K.int_shape(self.kernel))
        self.kernel = Concatenate(axis=2)([self.kernel_addition,self.kernel])
        print('concatenate kernel:',K.int_shape(self.kernel))
